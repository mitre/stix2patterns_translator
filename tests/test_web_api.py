...
from stix2patterns_translator.web_api import *
from tests.helpers.input_file_helpers import *
...
class TestRunFlask(unittest.TestCase):
    ...
    def input_files():
        ...
        for filename in listdir(path.join("tests", "input_files")):
            name, ext = path.splitext(filename) # reveal pattern name
            with open(path.join("tests", "input_files", filename), "r") as json_data:
